...
-(void) setValue:(id)value forKey:(NSString *)key
{
    ...
    if(self.testCaseIdentifier != nil && self.controlValue != nil && _hasSetShouldLoadImageFromUrl)
    {
...
-(void)configureBackgroundImage
{
    if([testCase isTesting])
    {
        if(shouldLoadNonControlFromUrl)
        {
            ...
            {
                [self setBackgroundImage:[imageCache imageWithUrl:[testCase value]] forState:UIControlStateNormal];
                [self addTarget: self action: @selector(registerTestResult) forControlEvents: UIControlEventTouchUpInside];
            }
            else
            {
                [self setBackgroundImage:[UIImage imageNamed:controlValue] forState:UIControlStateNormal];
            }
